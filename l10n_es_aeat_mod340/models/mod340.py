...
class L10nEsAeatMod340Report(orm.Model):
    ...
    def _get_number_records(self, cr, uid, ids, field_name, args, context):
        ...
        for model in self.browse(cr, uid, ids, context):
            result[model.id]['number_records'] = len(model.mapped('issued.tax_line_ids')) + len(model.mapped('received.tax_line_ids'))
            for issue in model.summary_issued:
                result[model.id]['total_taxable'] += issue.sum_base_amount
                result[model.id]['total_sharetax'] += issue.sum_tax_amount
                result[model.id]['total'] += issue.sum_base_amount + issue.sum_tax_amount
            for issue in model.summary_received:
                result[model.id]['total_taxable_rec'] += issue.sum_base_amount
                result[model.id]['total_sharetax_rec'] += issue.sum_tax_amount
                result[model.id]['total_rec'] += issue.sum_base_amount
                result[model.id]['total_rec'] += issue.sum_tax_amount
        return result
